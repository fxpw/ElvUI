...
local NP_Name = {
	enable = true,
	useClassColor = false,
	textFormat = '[namecolor][name:long]',
	position = 'TOPLEFT',
	parent = 'Health',
	xOffset = 0,
	yOffset = 2,
...
P.nameplates = {
	...
	engine = {
		loadDistance = 41,
		predictedHealthAndPower = true,
		dynamicScale = false,
		offsetY = 0,
		showClassColorFriendly = true,
		showNameClassColorFriendly = false,
		...
		showSelf = false,
		personalClickThrough = false,
		personalShowAlways = false,
		personalShowInCombat = false,
		personalShowWithTarget = 0,
		personalOffsetY = 0,
...
for unit, data in next, P.nameplates.units do
	if unit ~= 'TARGET' then
		...
		if unit == 'FRIENDLY_PLAYER' or unit == 'ENEMY_PLAYER' then
			data.pvpindicator = CopyTable(NP_PvPIndicator)
		end
...
P.nameplates.units.FRIENDLY_PLAYER.debuffs.priority = 'Blacklist,blockNoDuration,Personal,CCDebuffs'

P.nameplates.units.ENEMY_PLAYER.buffs.priority = 'Blacklist,PlayerBuffs,TurtleBuffs'
...
P.unitframe = {
	...
	units = {
		...
		raid10 = {
			...
			threatStyle = "GLOW",
			orientation = "MIDDLE",
			visibility = "[@raid11,exists]hide; [@raid6,exists]show; hide",
		...
		raid25 = {
			...
			threatStyle = "GLOW",
			orientation = "MIDDLE",
			visibility = "[@raid26,exists]hide; [@raid11,exists]show; hide",
			growthDirection = "RIGHT_DOWN",
			horizontalSpacing = 3,
			verticalSpacing = 3,
			numGroups = 5,
...
P.actionbar.keepButtonSizeRatio = true
