...
local function inviteClick(_, name)
	FriendDatatextRightClickMenu:Hide()
...
local levelNameClassStringCategory = "|T%s:16:16:0:0:64:64:4:60:4:60|t |cff%02x%02x%02x%d|r %s%s%s"
...
local function BuildFriendTable(total)
	...
	for i = 1, total do
		...
		if connected then
			...
			friendTable[i] = {name, level, className, area, connected, state, note, categoryIcon}
		end
	end
...
local function OnClick(_, btn)
	...
	if btn == "RightButton" then
		...
		EasyMenu(menuList, FriendDatatextRightClickMenu, "cursor", 0, 0, "MENU", 2)
	else
		...
		for i = 1, #friendTable do
			...
			if info[5] then
				...
				if not shouldSkip then
					...
					if info[8] then
						DT.tooltip:AddDoubleLine(format(levelNameClassStringCategory, info[8], levelc.r*255, levelc.g*255, levelc.b*255, info[2], info[1], groupedTable[grouped], " "..info[6]), info[4], classc.r, classc.g, classc.b, zonec.r, zonec.g, zonec.b)
					else
