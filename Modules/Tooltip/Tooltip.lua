...
function TT:GameTooltip_SetDefaultAnchor(tt, parent)
    ...
    if (not ((modifier2 == "SHIFT" and IsShiftKeyDown()) or (modifier2 == "CTRL" and IsControlKeyDown()) or
        (modifier2 == "ALT" and IsAltKeyDown()) or modifier2 == "NONE")) then
        tt:Hide()
        return
    end
    -- end
    ...
    if InCombatLockdown() and self.db.visibility.combat then
        ...
        if (not ((modifier == "SHIFT" and IsShiftKeyDown()) or (modifier == "CTRL" and IsControlKeyDown()) or
            (modifier == "ALT" and IsAltKeyDown()))) then
            tt:Hide()
            return
        end
    end
    ...
    if self.db.visibility.actionbars ~= "NONE" and ownerName and
        (find(ownerName, "ElvUI_Bar") or find(ownerName, "ElvUI_StanceBar") or find(ownerName, "PetAction")) and
        not keybindFrame.active then
        local modifier = self.db.visibility.actionbars

        if modifier == "ALL" or
            not ((modifier == "SHIFT" and IsShiftKeyDown()) or (modifier == "CTRL" and IsControlKeyDown()) or
                (modifier == "ALT" and IsAltKeyDown())) then
            tt:Hide()
            return
        end
    ...
    if anchor == nil or (ElvUI_ContainerFrame and anchor == ElvUI_ContainerFrame) or anchor == RightChatPanel or anchor ==
        ElvTooltipMover or anchor == _G.UIParent or anchor == E.UIParent then
        tt:ClearAllPoints()
...
function TT:SetUnitText(tt, unit, level, isShiftKeyDown)
    ...
    if UnitIsPlayer(unit) then
        ...
        if levelLine then
            ...
            levelLine:SetFormattedText("|cff%02x%02x%02x%s|r %s %s%s|r", diffColor.r * 255, diffColor.g * 255,
                diffColor.b * 255, level > 0 and level or "??", race or "", E:RGBToHex(color.r, color.g, color.b),
                localeClass)
        end
        if self.db.showElvUIUsers then
            ...
            if addonUser then
                ...
                -- print(addonUser)
                tt:AddDoubleLine(L["ElvUI Version:"], format('%.2f', addonUser), nil, nil, nil, same and 0.2 or 1,
                    same and 1 or 0.2, 0.2)
            end
        end
    else
        ...
        if levelLine then
            ...
            levelLine:SetFormattedText("|cff%02x%02x%02x%s|r%s %s%s", diffColor.r * 255, diffColor.g * 255,
                diffColor.b * 255, level > 0 and level or "??", classification[creatureClassification] or "",
                creatureType or "", pvpFlag)
        end
    end
...
function TT:INSPECT_TALENT_READY(event, unit)
    ...
    if not unitGUID or unitGUID ~= self.lastGUID then
        return
    end

    local _, specName = E:GetTalentSpecInfo(true)
    inspectCache[unitGUID] = {
        time = GetTime()
    }
    ...
    if not isShiftKeyDown and not isControlKeyDown and self.db.targetInfo then
        ...
        if unit ~= "player" and UnitExists(unitTarget) then
            ...
            if UnitIsPlayer(unitTarget) and not UnitHasVehicleUI(unitTarget) then
                ...
                targetColor = E.db.tooltip.useCustomFactionColors and
                                  E.db.tooltip.factionColors[UnitReaction(unitTarget, "player")] or
            ...
            tt:AddDoubleLine(format("%s:", TARGET), format("|cff%02x%02x%02x%s|r", targetColor.r * 255,
                targetColor.g * 255, targetColor.b * 255, UnitName(unitTarget)))
        end
        ...
        if inRaid or numParty > 0 then
            ...
            if numList > 0 then
                tt:AddLine(format("%s (|cffffffff%d|r): %s", L["Targeted By:"], numList, tconcat(targetList, ", ")),
                    nil, nil, nil, 1)
                twipe(targetList)
            end
        end
...
function TT:SetUnitAura(tt, ...)
    ...
    if id and self.db.spellID then
        if caster then
            ...
            tt:AddDoubleLine(format("|cFFCA3C3C%s|r %d", ID, id),
...
function TT:Initialize()
    ...
    GameTooltip.StatusBar.text:FontTemplate(E.Libs.LSM:Fetch("font", self.db.healthBar.font),
        self.db.healthBar.fontSize, self.db.healthBar.fontOutline)
