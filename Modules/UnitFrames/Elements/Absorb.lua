...
function UF:Construct_HealComm(frame)
	...
	UF:SetFrameLevel_HealComm(prediction, 12)
...
function UF:UpdateHealComm(_, myIncomingHeal, otherIncomingHeal, absorb, _, hasOverAbsorb, hasOverHealAbsorb, health, maxHealth)
	...
	if not pred.anchor then
		UF:Configure_HealComm(frame) -- workaround to db.enable returning false for some reason on configure_HealComm initial run
	end
...
-- UF.HealthClipFrame_HealComm = ARModule.HealthClipFrame_HealComm
-- UF.SetAlpha_HealComm = ARModule.SetAlpha_HealComm
-- UF.Construct_HealComm = ARModule.Construct_HealComm
-- UF.Configure_HealComm = ARModule.Configure_HealComm
-- UF.UpdateHealComm = ARModule.UpdateHealComm
