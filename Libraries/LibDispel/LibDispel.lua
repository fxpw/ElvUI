...
-- 3.3.5a Sirus: project globals don't exist; force Retail/Wrath false to use vanilla/TBC/WotLK spell branches
local Retail = WOW_PROJECT_ID and WOW_PROJECT_MAINLINE and (WOW_PROJECT_ID == WOW_PROJECT_MAINLINE) or false
local Wrath = WOW_PROJECT_ID and WOW_PROJECT_WRATH_CLASSIC and (WOW_PROJECT_ID == WOW_PROJECT_WRATH_CLASSIC) or false

local next = next
local GetCVar, SetCVar = GetCVar, SetCVar
-- Shim: 3.3.5a has IsSpellKnown(spellID, isPet) but not IsSpellKnownOrOverridesKnown / IsPlayerSpell
local IsSpellKnownOrOverridesKnown = IsSpellKnownOrOverridesKnown or IsSpellKnown or function() return false end
local IsPlayerSpell = IsPlayerSpell or function() return false end

-- 3.3.5a may not provide CopyTable global
local CopyTable = CopyTable or function(src) local t = {} for k, v in pairs(src) do t[k] = v end return t end
...
if Retail then
	...
	BleedList[377029] = "Severing Slice"
...
do
	...
	if myClass == 'WARLOCK' then
		if frame.RegisterUnitEvent then frame:RegisterUnitEvent('UNIT_PET', 'player') else frame:RegisterEvent('UNIT_PET') end
	end
