...
local UnitFactionGroup = function (unit) return select(2,UnitFactionGroup(unit)) end
...
local function isNameplatePerUnitTagEvent(parent, event)
	...
	return event and event:find('^UNIT_', 1, true) ~= nil
end

local function registerEvent(fontstr, event)
	-- Nameplates: unit events are registered per-unit via NP:RegisterAuraUnitEvents
